...
// Historical banding for the time machine: cuts come from the full 2020–2024
// distribution of this city's road-month values, so counts move smoothly with
// the seasons instead of flapping around a present-day threshold.
function timeCuts() { return S.timeCuts || cuts(); }
...
function smoothValueAt(series, i) {
  let s = 0, n = 0;
  for (let j = Math.max(0, i - 2); j <= Math.min(i, series.length - 1); j++) {
    if (series[j] != null) { s += series[j]; n++; }
  }
  return n ? s / n : valueAt(series, i);
}
...
// fitBounds with padding clamped to the canvas size (avoids "cannot fit" warnings
// on small windows) and a safe fallback.
function safeFit(bounds, padding, opts = {}) {
  const el = map.getContainer();
  const w = el.clientWidth || 1200, h = el.clientHeight || 700;
  const p = { top: 20, bottom: 20, left: 20, right: 20, ...padding };
  const horiz = p.left + p.right, vert = p.top + p.bottom;
  if (horiz > w - 100) {
    const k = Math.max(0, (w - 100)) / Math.max(1, horiz);
    p.left = Math.floor(p.left * k); p.right = Math.floor(p.right * k);
  }
  if (vert > h - 100) {
    const k = Math.max(0, (h - 100)) / Math.max(1, vert);
    p.top = Math.floor(p.top * k); p.bottom = Math.floor(p.bottom * k);
  }
  try { map.fitBounds(bounds, { padding: p, ...opts }); }
  catch { map.fitBounds(bounds, { padding: 20, ...opts }); }
}

function fitCity(animate = true) {
  const [w, s, e, n] = S.data.cities[S.city].bbox;
  safeFit([[w, s], [e, n]], { top: 40, bottom: 90, left: 360, right: 60 }, { duration: animate ? 900 : 0 });
}
...
function applyRoadStates() {
  ...
  S.roads.forEach((r, i) => {
    ...
    if (S.mode === "budget") {
      ...
      const v = smoothValueAt(r.trend, Math.round(S.monthIdx));
      const band = bandOf(v);
...
function onRoadHover(e) {
  ...
  let timeRow = "";
  if (S.mode === "time") {
    const band = bandOf(smoothValueAt(r.trend, Math.round(S.monthIdx)));
...
function prepareCity() {
  ...
  // Time-machine bands from the city's full observed history.
  const allVals = [];
  for (const r of S.roads) for (const v of r.trend) if (v != null) allVals.push(v);
  allVals.sort((a, b) => a - b);
  const q = (p) => allVals.length ? allVals[Math.min(allVals.length - 1, Math.floor(p * allVals.length))] : 0;
  S.timeCuts = { high: q(0.85), medium: q(0.5) };

  // Per-sector "exceptional stress" thresholds: each city tile judged against
  // its own 2020–2024 history (the tile grid is the data's true resolution).
  S.tileThresholds = S.data.cities[S.city].tiles.map((t) => {
    const vals = t.series.filter((v) => v != null).slice().sort((a, b) => a - b);
    return vals.length ? vals[Math.floor(0.85 * vals.length)] : Infinity;
  });
...
function selectRoad(r) {
  ...
  if (S.mapReady) {
    ...
    safeFit([[minx, miny], [maxx, maxy]], { top: 80, bottom: 120, left: 380, right: 440 }, { maxZoom: 15, duration: 900 });
  }
...
function hotCountAt(i) {
  ...
  let n = 0;
  tiles.forEach((t, k) => {
    const v = smoothValueAt(t.series, i);
    if (v != null && v >= S.tileThresholds[k]) n++;
  });
  return n;
}
...
function updateStoryRoad() {
  ...
  const r = S.selected;
  const i = Math.round(S.monthIdx);
  const v = smoothValueAt(r.trend, i);
  const band = bandOf(v);
