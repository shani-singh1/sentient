...
def _scale_landsat_l2(image: "object") -> "object":
    optical = image.select(["SR_B4", "SR_B3", "SR_B2"]).multiply(0.0000275).add(-0.2)
    thermal = image.select(["ST_B10"]).multiply(0.00341802).add(149.0)
    return optical.addBands(thermal).rename(["red", "green", "blue", "thermal_k"])


def fetch_landsat_monthly(
    city: str,
    ...
    for idx, (year, month) in enumerate(months_list, 1):
        ...
        print(f"[Landsat] {ym} ({idx}/{total}) ...", flush=True)
        ...
        collection = (
            ...
            .map(_scale_landsat_l2)
        )
        count = int(collection.size().getInfo())
        if count <= 0:
            print(f"[Landsat] {ym} skip (no scenes)", flush=True)
            continue

        out_dir = ensure_dir(RAW_ROOT / "landsat" / f"{year:04d}" / f"{month:02d}")
        out_path = out_dir / f"landsat_composite_{city_key}_{year:04d}{month:02d}.tif"
        if skip_existing and out_path.exists():
            print(f"[Landsat] {ym} skip (exists)", flush=True)
            out_files.append(out_path)
            continue
        monthly = collection.median().clip(region).toFloat()
        export_image_to_geotiff(ee, monthly, region, out_path, scale=scale, crs="EPSG:4326")
        out_files.append(out_path)
        print(f"[Landsat] {ym} done", flush=True)

    return out_files


def main() -> None:
    parser = argparse.ArgumentParser(description="Ingest Landsat monthly composite GeoTIFFs via GEE.")
    ...
    parser.add_argument("--no-skip-existing", action="store_true", help="Re-download even if output file exists")
    ...
    meta = build_metadata(
        ...
        files=files,
        crs="EPSG:4326",
        notes=f"Landsat L2 monthly median composites from GEE (LC08+LC09, scale={args.scale}m).",
    )
    append_metadata(meta)
    print(f"wrote {len(files)} landsat monthly GeoTIFFs")
